...
4. **Honesty bar:** never add a performance/benchmark/marketing claim that isn't backed by a committed, reproducible measurement (see the 0.9.2 benchmark correction). Use [current evidence and methodology](../docs/PERFORMANCE.md); older best-of results and the new paired evaluator are not interchangeable before/after measurements.
...
- [ ] **G2.1 — Performance/allocation evaluator. IN PROGRESS** on `codex/g2-evaluator`. First slice: paired monotonic native-process collector, per-child RSS, hierarchical bootstrap, fail-closed qualification, retained raw evidence, independent output oracles and manifest preserving all11 planned cases. Only fib and wordcount are runnable in this slice; wordcount is labeled an application comparison because top20 algorithms differ. Allocation/RC/live-byte instrumentation, remaining fixtures/held-out corpus, in-program batching and cross-host evidence remain required. Controlled fixture forms remain pending G3, not silently passed. See `benchmarks/EVALUATOR.md`; all broader G1–G8 outcomes remain open.
  ...
  - **Unresolved G1 native Unicode parity defects exposed during E2:** JIT/AOT differ for `upper("Straße")`, `lower("É")`, NBSP trimming and empty-separator UTF-8 splitting. Actual probe outputs and reproduction steps are retained in [EVALUATOR.md](../benchmarks/EVALUATOR.md#known-unicode-parity-work-not-covered-by-the-token-fixture). These must be corrected under the native semantic-parity work; the bounded token fixture does not certify or close them. No runtime behavior was changed by the string benchmark slice.
