...
fn rt_cstr_arg(ptr: *const u8) -> String {
    if ptr.is_null() {
        String::new()
    } else {
...
fn rt_json_escape(value: &str) -> String {
    value
        .replace('\\', "\\\\")
        .replace('"', "\\\"")
        .replace('\n', "\\n")
...
fn rt_extract_after_marker(body: &str, marker: &str) -> Option<String> {
...
fn rt_agent_context_block(
    ...
    if !system.trim().is_empty() {
        merged.push_str(system.trim());
        merged.push_str("\n\n");
    }
    if !tools.trim().is_empty() {
        merged.push_str("Available tools: ");
        merged.push_str(tools.trim());
        merged.push('\n');
    }
    if !resources.trim().is_empty() {
        merged.push_str("Available resources: ");
        merged.push_str(resources.trim());
        merged.push('\n');
    }
    if !prompts.trim().is_empty() {
        merged.push_str("Available prompts: ");
        merged.push_str(prompts.trim());
        merged.push('\n');
    }
    if !output_type.trim().is_empty() {
        merged.push_str("Expected output type: ");
        merged.push_str(output_type.trim());
        merged.push('\n');
    }
    if !output_schema.trim().is_empty() {
        merged.push_str("Expected output schema JSON: ");
        merged.push_str(output_schema.trim());
        merged.push('\n');
    }
    merged.push_str("\nUser request:\n");
...
fn rt_join_string_array(ptr: *const u8) -> String {
    if ptr.is_null() {
        return String::new();
    }
    ...
    for idx in 0..len as usize {
        let elem_ptr = unsafe { *((ptr as *const i64).add(1 + idx) as *const *const u8) };
        out.push(rt_cstr_arg(elem_ptr));
    }
    out.join(", ")
}

type ToolWrapperFn = extern "C" fn(*const u8) -> *const u8;

fn rt_join_tool_wrapper_names(ptr: *const u8) -> Vec<String> {
    if ptr.is_null() {
        return Vec::new();
    }
    let len = unsafe { *(ptr as *const i64) };
    let mut out = Vec::new();
    for idx in 0..len.max(0) as usize {
        let elem_ptr = unsafe { *((ptr as *const i64).add(1 + idx) as *const *const u8) };
        out.push(rt_cstr_arg(elem_ptr));
    }
    out
...
fn rt_join_tool_wrapper_ptrs(ptr: *const u8) -> Vec<usize> {
    if ptr.is_null() {
        return Vec::new();
    }
    let len = unsafe { *(ptr as *const i64) };
    let mut out = Vec::new();
    for idx in 0..len.max(0) as usize {
        let elem_val = unsafe { *((ptr as *const i64).add(1 + idx)) };
        out.push(elem_val as usize);
    }
    out
}

fn rt_parse_tool_directive(response: &str) -> Option<(String, String)> {
    let rest = response.strip_prefix("__CALL_TOOL__:")?;
    let (tool, arg) = rest.split_once(':').unwrap_or((rest, ""));
    Some((tool.trim().to_string(), arg.to_string()))
}

fn rt_provider_request(provider: &str, model_name: &str, prompt: &str) -> Result<String, String> {
    match provider {
        "mock" => {
            if model_name == "echo" {
                return Ok(prompt
            ...
            if model_name == "structured" {
                let user_prompt = prompt
                    ...
                    .map(|(_, tail)| tail.to_string())
                    .unwrap_or_else(|| prompt.to_string());
                return Ok(format!("{{\"mock\":\"{}\"}}", rt_json_escape(&user_prompt)));
            }
            if model_name == "structured_str" {
                let user_prompt = prompt
                    .rsplit_once("User request:\n")
                    .map(|(_, tail)| tail.to_string())
                    .unwrap_or_else(|| prompt.to_string());
                return Ok(format!("\"{}\"", rt_json_escape(&user_prompt)));
            }
            if let Some(tool_name) = model_name.strip_prefix("toolloop:") {
                if let Some(pos) = prompt.rfind("Tool result for ") {
                    let tool_result = prompt[pos..]
                        .split_once('\n')
                        .map(|(_, tail)| tail.to_string())
                        .unwrap_or_default();
                    return Ok(tool_result.lines().next().unwrap_or_default().to_string());
                }
                let user_prompt = prompt
                    .rsplit_once("User request:\n")
                    .map(|(_, tail)| tail.to_string())
                    .unwrap_or_else(|| prompt.to_string());
                return Ok(format!("__CALL_TOOL__:{tool_name}:{user_prompt}"));
            }
            Err(format!("error: unsupported mock model `{model_name}`"))
        }
        "openai" => {
            let api_key = std::env::var("OPENAI_API_KEY")
                .map_err(|_| "error: missing OPENAI_API_KEY".to_string())?;
            let body = format!(
                "{{\"model\":\"{}\",\"messages\":[{{\"role\":\"system\",\"content\":\"{}\"}},{{\"role\":\"user\",\"content\":\"{}\"}}]}}",
                rt_json_escape(model_name),
                rt_json_escape("You are a Turbo agent runtime. Reply with either __CALL_TOOL__:<tool_name>:<arg> or a final answer."),
                rt_json_escape(prompt)
            );
            ...
            Ok(rt_extract_after_marker(&body, "\"content\":\"").unwrap_or(body))
        }
        "anthropic" => {
            let api_key = std::env::var("ANTHROPIC_API_KEY")
                .map_err(|_| "error: missing ANTHROPIC_API_KEY".to_string())?;
            let body = format!(
                "{{\"model\":\"{}\",\"max_tokens\":1024,\"messages\":[{{\"role\":\"user\",\"content\":\"{}\"}}]}}",
                rt_json_escape(model_name),
                rt_json_escape(&format!(
                    "You are a Turbo agent runtime. Reply with either __CALL_TOOL__:<tool_name>:<arg> or a final answer.\n\n{}",
                    prompt
                ))
            );
            let output = std::process::Command::new("curl")
                ...
                .map_err(|e| format!("error: {e}"))?;
            let body = String::from_utf8_lossy(&output.stdout).to_string();
            Ok(rt_extract_after_marker(&body, "\"text\":\"").unwrap_or(body))
        }
        _ => Err(format!("error: unsupported provider `{provider}`")),
    }
}
...
pub(crate) extern "C" fn rt_agent_ask(agent_ptr: *const u8, prompt: *const u8) -> *const u8 {
    if agent_ptr.is_null() {
        return rt_owned_cstr("error: null agent".to_string());
    }

    let load_slot = |slot: usize| unsafe { *(agent_ptr.add(slot * 8) as *const *const u8) };

    let model = rt_cstr_arg(load_slot(0));
    let system = rt_cstr_arg(load_slot(1));
    let tools = rt_join_string_array(load_slot(2));
    let resources = rt_join_string_array(load_slot(3));
    let prompts = rt_join_string_array(load_slot(4));
    let output_type = rt_cstr_arg(load_slot(5));
    let output_schema = rt_cstr_arg(load_slot(6));
    let tool_wrapper_names = rt_join_tool_wrapper_names(load_slot(2));
    let tool_wrapper_ptrs = rt_join_tool_wrapper_ptrs(load_slot(7));
    let prompt = rt_cstr_arg(prompt);

    let (provider, model_name) = rt_detect_provider(&model);
    if provider == "mock" {
        return rt_owned_cstr(match model_name {
            "echo" => prompt,
            "system" => system,
            ...
            tool_name if tool_name.starts_with("tool:") => {
                ...
                if let Some(index) = tool_wrapper_names.iter().position(|name| name == requested) {
                    if let Some(wrapper_ptr) = tool_wrapper_ptrs.get(index).copied() {
                        if wrapper_ptr != 0 {
                            let arg = std::ffi::CString::new(prompt.clone()).unwrap();
                            let func: ToolWrapperFn = unsafe { std::mem::transmute(wrapper_ptr) };
                            return func(arg.as_ptr() as *const u8);
                        }
                    }
                }
                format!("error: unknown mock tool `{requested}`")
            }
            toolloop if toolloop.starts_with("toolloop:") => {
                let _ = toolloop;
                let mut transcript = rt_agent_context_block(
                    &system,
                    &tools,
                    &resources,
                    &prompts,
                ...
                for _ in 0..4 {
                    let response = match rt_provider_request(provider, model_name, &transcript) {
                        Ok(resp) => resp,
                        Err(err) => return rt_owned_cstr(err),
                    };
                    if let Some((tool_name, tool_arg)) = rt_parse_tool_directive(&response) {
                        if let Some(index) = tool_wrapper_names
                            .iter()
                            .position(|name| name == &tool_name)
                        {
                            let Some(wrapper_ptr) = tool_wrapper_ptrs.get(index).copied() else {
                                return rt_owned_cstr(format!(
                                    "error: missing tool wrapper for `{tool_name}`"
                                ));
                            };
                            if wrapper_ptr == 0 {
                                return rt_owned_cstr(format!(
                                    "error: tool `{tool_name}` has an unsupported executable signature"
                                ));
                            }
                            let arg_cstr = std::ffi::CString::new(tool_arg.clone())
                                .unwrap_or_else(|_| std::ffi::CString::new("").unwrap());
                            let func: ToolWrapperFn = unsafe { std::mem::transmute(wrapper_ptr) };
                            let tool_result = rt_cstr_arg(func(arg_cstr.as_ptr() as *const u8));
                            transcript.push_str(&format!(
                                "\nTool result for {}:\n{}\nRespond with either another __CALL_TOOL__ or a final answer.",
                                tool_name, tool_result
                            ));
                            continue;
                        } else {
                            return rt_owned_cstr(format!("error: unknown tool `{tool_name}`"));
                        }
                    }
                    return rt_owned_cstr(response);
                }
                return rt_owned_cstr("error: tool loop exceeded max steps".to_string());
            }
            _ => format!("mock:{model_name}:{prompt}"),
        });
    }
    ...
    for _ in 0..4 {
        ...
        if let Some((tool_name, tool_arg)) = rt_parse_tool_directive(&response) {
            ...
            {
                let Some(wrapper_ptr) = tool_wrapper_ptrs.get(index).copied() else {
                    return rt_owned_cstr(format!("error: missing tool wrapper for `{tool_name}`"));
                };
                if wrapper_ptr == 0 {
                    return rt_owned_cstr(format!(
                        "error: tool `{tool_name}` has an unsupported executable signature"
    ...
    rt_owned_cstr("error: tool loop exceeded max steps".to_string())
}

pub(crate) extern "C" fn rt_agent_ask_structured(
    agent_ptr: *const u8,
    prompt: *const u8,
) -> *const u8 {
    if agent_ptr.is_null() {
    ...
    let output_type = rt_cstr_arg(unsafe { *((agent_ptr.add(5 * 8)) as *const *const u8) });
    let output_schema = rt_cstr_arg(unsafe { *((agent_ptr.add(6 * 8)) as *const *const u8) });
    let prompt = rt_cstr_arg(prompt);
    let merged = format!(
        "{}\nReturn only valid JSON for output type `{}` matching this schema:\n{}",
        prompt, output_type, output_schema
    );
    let merged_cstr =
        std::ffi::CString::new(merged).unwrap_or_else(|_| std::ffi::CString::new("").unwrap());
    rt_agent_ask(agent_ptr, merged_cstr.as_ptr() as *const u8)
}

pub(crate) extern "C" fn rt_agent_stream(agent_ptr: *const u8, prompt: *const u8) -> *mut u8 {
    let answer_ptr = rt_agent_ask(agent_ptr, prompt);
    let answer = rt_cstr_arg(answer_ptr);
    let chunks: Vec<String> = if answer.is_empty() {
        Vec::new()
    } else {
        answer.split_whitespace().map(|s| s.to_string()).collect()
    };
    let arr = rt_array_alloc(chunks.len() as i64);
    for (idx, chunk) in chunks.iter().enumerate() {
        let raw = rt_owned_cstr(chunk.clone()) as i64;
        unsafe {
            *((arr as *mut i64).add(1 + idx)) = raw;
        }
    }
    arr
}
