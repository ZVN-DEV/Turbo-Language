...
pub(crate) struct Ctx<'a, M: Module> {
    ...
    /// Per-concrete-`V` release thunks for typed `HashMap<K, V>` values that
    /// need deeper cleanup than a plain top-level `rt_release`.
    pub(crate) hashmap_value_release_thunks: &'a HashMap<String, FuncId>,
