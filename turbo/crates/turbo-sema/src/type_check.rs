...
//!
//! This module contains the bulk of the `Checker`'s work:
//! * `check_module` — top-level driver; registers types/functions and then
//!   walks every function body.
//! * `check_function` / `check_function_with_name` — per-function scope
//!   setup and body checking.
//! * `check_expr` / `check_expr_inner` — recursive expression type
//!   inference, including the large match-arm that handles every
//!   [`turbo_ast::Expr`] variant.
//! * `check_stmt` — statement checking (let, return, defer, destructure).
//! * Small helpers: `is_builtin_function`, `root_var_name`,
//!   `substitute_ty`, `substitute_return_type`, `inject_constants`.
//!
